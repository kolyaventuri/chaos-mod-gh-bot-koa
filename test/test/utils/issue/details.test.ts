...
test('calls #getIssueProblems', t => {
  getIssueDetails({issue: {title: 'abc', body: '123', labels: []}} as unknown as Issue);

  t.true(getIssueProblems.calledWith({title: 'abc', body: '123', type: IssueType.UNKNOWN}));
});

test('returns the problems, and the status', t => {
  const result = getIssueDetails({issue: {title: 'abc', body: '123', labels: []}} as unknown as Issue);
